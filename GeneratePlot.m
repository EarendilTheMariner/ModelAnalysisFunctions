function GeneratePlot(MouseNet)
    screensize = get(groot,'ScreenSize');

    ConnMat = MouseNet.ConnMat;
    
    Ex =  MouseNet.Transmit > 0;
    In =  MouseNet.Transmit < 0;
    Dist = bsxfun(@minus,MouseNet.Position(:,2),MouseNet.Position(:,2)');
    ...
    L = round(range(MouseNet.Position(:,2)),-2);

function Stats = R0LSSweep(varargin)

    LSSweep = linspace(0.5,10,200);
    Summarys = zeros(1,length(LSSweep));
    Caudal = false;
    Bidi = false;
    ...
    for i = 1:length(LSSweep)
        close all 
        clearvars -except Summarys i LSSweep MouseNet Caudal Bidi

        if Caudal
            NetParams = NetworkParameters();
            NetParams.AddCelltype('V2a-1','cau','','','','','',1000*LSSweep(i),0.1*1.8);
        ...
        elseif Bidi
            ...
            NetParams.AddCelltype('V1','bi','','','','','',3300*LSSweep(i),0.1*1.8);
            MouseNet = Network(3000,0.5,NetParams);   
            MouseNet.Simulate(10000);
        ...
        subplot(2,2,1);
        scatter(LSSweep(i)*3300,Summarys(1:i))
        xlim([0,35000]);
        ylim([0,500]);
        xlabel('Bidirectional Length Scale (Iteration)');
